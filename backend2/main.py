...
NON_ANSWERS = {
    "i don't know", "idk", "sorry i don't know", "no idea", "no answer",
    "n/a", "none", "don't know", "no answer provided", "sorry", "not sure",
    "i dont know", "dont know", "sorry i dont know"
}

def is_valid_candidate_answer(ans: Optional[str]) -> bool:
    if not ans:
        return False
    clean = ans.strip().lower().rstrip(".!")
    if not clean or clean in NON_ANSWERS:
        return False
    return True
...
def run_evaluation_crew(session_id: str) -> tuple[float, List[QuestionEvaluationItem]]:
    ...
    if not valid_questions_to_eval:
        ...
        save_question_scores_db(session_id, all_evals)
        save_session_total_score_db(session_id, 0.0)
    ...
    evaluator_agent = Agent(
        ...
        goal="Evaluate candidate answers against ideal reference answers using a strict 4-step evaluation process and numerical rubric.",
        backstory="You are a veteran technical interviewer. You evaluate candidate responses carefully against reference answers, assigning scores from 0 to 10 based on an explicit scoring rubric.",
        verbose=True,
    ...
    eval_task = Task(
        description=(
            ...
            "CRITICAL RULE:\n"
            "• NEVER say the answer is empty if Candidate Submitted Answer contains any text.\n\n"
            "EXPLICIT SCORING RUBRIC:\n"
            "• 0 = Empty answer\n"
            "• 1-2 = Wrong answer\n"
            "• 3-5 = Partial answer\n"
            "• 6-8 = Mostly correct answer\n"
    ...
    for q in valid_questions_to_eval:
        ...
        if llm_item and float(llm_item.score) >= 1.0:
            score_val = max(1.0, min(10.0, float(llm_item.score)))
        ...
        elif llm_item and llm_item.feedback and "empty" not in llm_item.feedback.lower() and float(llm_item.score) > 0:
            score_val = float(llm_item.score)
            feedback_text = llm_item.feedback.strip()
        else:
            # Safeguard heuristic if LLM hallucinated score 0 on valid answer
            if word_count >= 30:
                score_val = 7.5
                ...
                score_val = 4.0
                feedback_text = "Concise answer provided; needs deeper technical elaboration."
    ...
    save_question_scores_db(session_id, all_evaluations)
...
def load_role_config(role_input: str) -> dict:
    ...
    if not roles_file.exists():
        roles_file = Path(__file__).parent / "config" / "roles.json"
...
def run_interview_crew(role: str, experience: int, difficulty: str, total_questions: int) -> List[GeneratedQuestion]:
    ...
    # 1. Planner Agent
    planner_agent = Agent(
        role="Interview Curriculum Planner",
        goal=f"""
            Create an interview blueprint for the role '{role_config['display_name']}'.
    ...
    question_agent = Agent(
        role=f"Technical Question Creator for {role_config['display_name']}",
        goal=f"Generate {total_questions} distinct technical interview questions based on the planner's topic blueprint.",
        backstory=f"You are a Principal Engineer in {role_config['display_name']}. You craft realistic, clear, and challenging questions spanning core concepts, system design, coding standards, and scenario-based problem solving.",
        verbose=True,
        allow_delegation=False,
        llm=llm
    )
    ...
    # 3. Answer Agent
    answer_agent = Agent(
        role=f"Subject Matter Answer Specialist in {role_config['display_name']}",
        goal=f"Formulate ideal, comprehensive, and accurate reference answers for each generated interview question.",
        backstory=f"You are a Domain Expert in {role_config['display_name']}. You write clear, complete, and exemplary solutions explaining technical principles and best practices.",
        verbose=True,
        allow_delegation=False,
        llm=llm
    )
    ...
    # 4. QA Agent
    qa_agent = Agent(
        role="Interview QA & Quality Reviewer",
    ...
    question_task = Task(
        description=f"""Read the planner blueprint from plan_task.
            Generate exactly {total_questions} technical interview questions for a {role_config['display_name']} candidate with {experience} years experience at {difficulty} level.
            Requirements:
            Mandatory Skills:\n- {mandatory}
            Excluded Skills:\n- {excluded}
            Rules:
